parse.bill <- function(file) {  
    billinfo <- function(t0) {
        ##  function to get bill info (type, year, number)
    ...
    bill <- data.frame(do.call(rbind,sapply(billinfo(t0), strsplit, split="/|-")))
    ...
    res <- try(data.frame(billtype=bill[,1],
                          billno=bill[,2],
                          billyear=bill[,3],
                          billid=billid,
                          propno=f(propno),
